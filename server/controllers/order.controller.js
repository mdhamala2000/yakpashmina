...
const syncProductPrices = async (products) => {
    ...
    const syncedProducts = products.map((item) => {
        ...
        const perUnit = quantity > 0 ? price / quantity : price;
        return {
            ...item,
            perUnit: perUnit,
            price: price,
...
export const createOrderController = async (request, response) => {
    try {
        ...
        let order = new OrderModel({
            ...
            subTotal: request.body.subTotal || request.body.totalAmt || request.body.totalAmount,
        ...
        for (let i = 0; i < request.body.products.length; i++) {
            ...
            const newStock = Math.max(0, parseInt(request.body.products[i].countInStock - request.body.products[i].quantity));
...
export const captureOrderPaypalController = async (request, response) => {
    try {
        ...
        // Try to capture the order via PayPal API
        try {
            ...
            console.log('PayPal capture result:', paypalCaptureData);
        } catch (captureError) {
            console.log('PayPal capture error (order may already be captured):', captureError.message);
        }
        ...
        const orderInfo = {
            ...
            delivery_address: request.body.delivery_address,
            totalAmt: request.body.totalAmt || request.body.totalAmount || request.body.subTotal,
            subTotal: request.body.subTotal || request.body.totalAmt || request.body.totalAmount,
            shippingCost: request.body.shippingCost || 0,
            date: request.body.date,
    ...
    try {
        ...
        return response.status(200).json({
            ...
            order: {
                ...
                status: order.orderStatus || order.payment_status || 'pending',
                createdAt: order.date,
                processedAt: order.processedDate,
                shippedAt: order.shippedDate,
                outForDeliveryAt: order.outForDeliveryDate,

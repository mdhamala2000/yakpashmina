...
const AddProductMaterials = () => {
   ...
   const getData=()=>{
    ...
    const handleSubmit = (e) => {
        ...
        if (name === "") {
            context.alertBox("error", "Please enter product material");
            return false;
        }
        ...
        if(editId===""){
            ...
            postData(`/api/product/productMaterials/create`, {
                name: name
            }).then((res) => {
                if(res?.error===false){
        ...
        if(editId!==""){
            editData(`/api/product/productMaterials/${editId}`, {
                name: name
            }).then((res) => {
    ...
    const deleteItem = (id) => {
        deleteData(`/api/product/productMaterials/${id}`).then((res) => {
            getData();
    ...
    const editItem=(id)=>{
        fetchDataFromApi(`/api/product/productMaterials/${id}`).then((res)=>{
            ...
            <div className="card my-4 pt-5 pb-5 shadow-md sm:rounded-lg bg-white w-[100%] sm:w-[100%] lg:w-[65%]">
                <form className='form py-3 p-6' onSubmit={handleSubmit}>
                    <div className='col mb-4'>
                        <h3 className='text-[14px] font-[500] mb-1 text-black'>PRODUCT MATERIAL</h3>
...
export default AddProductMaterials;